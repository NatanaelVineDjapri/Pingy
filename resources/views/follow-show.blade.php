            ...
            <div class="tweet-item">
                ...
                <div class="tweet-body">
                    @forelse ($user->following as $followingUser)
                        <div class="followers-header">
                            @if ($followingUser->avatar)
                                <img
                                    src="{{ asset('storage/' . $followingUser->avatar) }}"
                                    class="avatar"
                                />
                            @else
                                <img
                            ...
                            <form
                                action="{{ route('follow', $followingUser) }}"
                                method="POST"
                    ...
                    @empty
                        @if (auth()->id() === $user->id)
                            <p class="tweet-text">
                                Start following people to discover amazing content!
                            </p>
                        @else
                ...
                <div class="tweet-body">
                    @forelse ($user->followers as $follower)
                        <div class="followers-header">
                            @if ($follower->avatar)
                                <img
                                    src="{{ asset('storage/' . $follower->avatar) }}"
                                    class="avatar"
                                />
                            @else
                                <img
                    ...
                    @empty
                        @if (@auth()->id() === $user->id)
                            <p class="tweet-text">
                                Invite others to follow you and share your journey!
                            </p>
                        @else
                            <p class="tweet-text">
                                This user doesnt have any followers yet.Be their first follower!
                            </p>
                        @endif
                    @endforelse
                </div>
