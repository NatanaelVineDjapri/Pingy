                ...
                @foreach ($likeTweets as $like)
                    <div class="tweet">
                        <div>
                            @if ($like->user->avatar)
                                <img
                                    src="{{ asset('storage/' . $like->user->avatar) }}"
                                    class="avi"
                                />
                            @else
                        ...
                        <div class="tweetbody">
                            ...
                            @if ($like->tweetImage)
                                <div class="tweet-image">
                                    <img
                                        src="{{ asset('storage/' . $like->tweetImage) }}"
                                        alt="Tweet image"
                                        style="
                                            max-width: 80%;
                                            border-radius: 10px;
                            ...
                            <ul class="retweeticons">
                                <li>
                                    <a href="{{ route('showcomment', ['tweet' => $like->id]) }}">
                                ...
                                <li>
                                    @php
                                        $retweet = auth()
                                            ->user()
                                            ->retweetTweets->contains($like->id);
                                    @endphp

                                    <form
                                        action="{{ route('postretweet', $like->id) }}"
                                        method="POST"
                                    >
                                        @csrf
                                        @if ($retweet)
                                ...
                                <li>
                                    @php
                                        $liked = auth()
                                            ->user()
                                            ->likedTweets->contains($like->id);
                                    @endphp

                                    <form
                                        action="{{ route('liketweet', $like->id) }}"
                                        method="POST"
                                    >
                                        @csrf
                                        @if ($liked)
